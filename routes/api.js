...
const listkey = ["yanzzlolkey", "zeeoneofc"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'yanzzlolkey') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'yanzzlolkey')  return res.json(loghandler.invalidKey)
